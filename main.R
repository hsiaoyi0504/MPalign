...
# cell_features_uninfected$artificial_batch <- sample(c("batch 1", "batch 2"), length(cell_features_uninfected$Image_Metadata_PlateID), replace = TRUE)
# cds <- prepare_cds(cell_features_uninfected, verbose=TRUE, artificial_batch=TRUE)
# cds <- preprocess_cds(cds, num_dim = 100)
# cds <- reduce_dimension(cds)
# jpeg('before_remove_batch_effect.jpg', width=2000, height=2000, res=300)
# print(plot_cells(cds, color_cells_by="artificial_batch", label_cell_groups=FALSE))
# dev.off()
# cds <- align_cds(cds, num_dim = 100, alignment_group = "artificial_batch")
# cds <- reduce_dimension(cds)
# jpeg('after_remove_batch_effect.jpg', width=2000, height=2000, res=300)
# print(plot_cells(cds, color_cells_by="artificial_batch", label_cell_groups=FALSE))
# dev.off()
# cds <- cluster_cells(cds, resolution=1e-5)
#
# temp <- data.frame(colData(cds)$artificial_batch, clusters(cds))
# colnames(temp) <- c("artificial.batch", "cluster")
# count_batch_1 <- temp %>% filter(artificial.batch == "batch 1") %>% count(cluster, sort=TRUE)
# count_batch_2 <- temp %>% filter(artificial.batch == "batch 2") %>% count(cluster, sort=TRUE)
# p <- count_batch_1["n"] / sum(count_batch_1["n"])
